    ...
    <div className="px-5 py-6 pb-40 min-h-[calc(100vh-72px)]">
      ...
      <div className="flex items-center justify-between bg-white border border-[#e5e5e7] rounded-2xl p-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs mobile-text-muted">Delivery</p>
            <p className="text-sm font-medium mobile-text-dark truncate max-w-[150px]">
              {data.deliveryLocation === 'delivery' ? data.deliveryAddress : 'Office Pickup'}
            </p>
          </div>
        </div>
        <div className="text-right">
          <p className="text-xs mobile-text-muted">Total</p>
          <p className="text-lg font-bold mobile-text-dark">${total.toLocaleString()}</p>
        </div>
      </div>

      {/* Dates & Documents Summary */}
      <div className="grid grid-cols-2 gap-3 mb-6">
        <div className="bg-white border border-[#e5e5e7] rounded-xl p-3">
          <p className="text-xs mobile-text-muted mb-1">Dates</p>
        ...
        <div className="bg-white border border-[#e5e5e7] rounded-xl p-3">
          <p className="text-xs mobile-text-muted mb-1">License</p>
          <p className="text-sm font-medium mobile-text-dark truncate">{data.licenseNumber}</p>
        </div>
        <div className="bg-white border border-[#e5e5e7] rounded-xl p-3">
          <p className="text-xs mobile-text-muted mb-1">Passport</p>
          <p className="text-sm font-medium mobile-text-dark truncate">{data.passportNumber}</p>
        </div>
        <div className="bg-white border border-[#e5e5e7] rounded-xl p-3">
          <p className="text-xs mobile-text-muted mb-1">Duration</p>
          <p className="text-sm font-medium mobile-text-dark">{days} days</p>
        </div>
      </div>

      {/* Payment Methods */}
      <div className="bg-white border border-[#e5e5e7] rounded-2xl p-4 mb-6">
        <h3 className="font-semibold mb-4 mobile-text-dark">Payment Method</h3>
            ...
            <button
              ...
              className={cn(
                ...
                data.paymentMethod === method.id 
                  ? "border-[#161821] bg-[#f5f5f7]" 
                  : "border-[#e5e5e7]"
              )}
            >
              <div className={cn(
                "w-10 h-10 rounded-xl flex items-center justify-center",
                method.iconBg || "bg-[#f5f5f7]"
              )}>
                {typeof method.icon === 'string' ? (
                  <span className="text-white font-bold">{method.icon}</span>
                ) : (
                  <method.icon className={cn("h-5 w-5", method.iconColor || "mobile-text-dark")} />
                )}
              </div>
              <span className="flex-1 text-left font-medium mobile-text-dark">{method.name}</span>
              {data.paymentMethod === method.id && (
                <div className="w-5 h-5 rounded-full bg-[#161821] flex items-center justify-center">
        ...
        {showCardForm && data.paymentMethod === 'card' && (
          <div className="mt-4 pt-4 border-t border-[#e5e5e7] space-y-4">
            <div>
              <label className="text-sm mobile-text-muted mb-2 block">Card Number</label>
              <input
                type="text"
                value={data.cardNumber || ''}
                onChange={(e) => onChange({ cardNumber: e.target.value })}
                placeholder="0000 0000 0000 0000"
                className="w-full px-4 py-3 bg-[#f5f5f7] rounded-xl border border-[#e5e5e7] text-sm mobile-text-dark placeholder:text-[#8e8e93]"
              />
            </div>
            <div>
              <label className="text-sm mobile-text-muted mb-2 block">Card Holder</label>
              <input
                type="text"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-sm mobile-text-muted mb-2 block">Expiry</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-sm mobile-text-muted mb-2 block">CVV</label>
                <input
                  type="text"
                  value={data.cardCvv || ''}
      ...
      <label className="flex items-center gap-3 cursor-pointer mb-6">
        <div 
          className={cn(
            ...
            data.termsAccepted ? "bg-[#161821] border-[#161821]" : "border-[#8e8e93]"
          )}
          onClick={() => onChange({ termsAccepted: !data.termsAccepted })}
        >
          {data.termsAccepted && <Check className="h-3 w-3 text-white" />}
        </div>
        <span className="text-sm mobile-text-dark">
          I agree with{' '}
          <Link href="/mobile/terms" className="text-[#dd3155] underline">Terms & Conditions</Link>.
        </span>
      </label>
        ...
        <button
          ...
          className={cn(
            ...
            data.termsAccepted && !loading
              ? "bg-[#161821] text-white active:scale-[0.98]"
